import {
  ...
  const Header = () => {
    ...
    return (
      <div className="flex flex-row bg-primary p-4 space-x-6">
        <div className='ml-3 mr-8'>
        <Picture />
        </div>
        <div className="ml-8 flex flex-col justify-center self-center items-center space-y-2 text-center">
        <div>
  ...
  const Summary = () => {
    ...
    return (
      <section id={section.id} className="grid grid-rows-1 border-t">
  ...
  export const Newresume = ({ columns, isFirstPage = false }: TemplateProps) => {
  ...
  <div className='sidebar group col-span-1 space-y-4'>
        ...
        <div className="main group space-y-4 col-span-2">
          {main.map((section) => (
            <Fragment key={section}>{mapSectionToComponent(section)}</Fragment>
          ))}
        </div>
